      ...
      success(res){
        ...
        while(j<i){                        //遍历，选择相同id的数据，并取出
          if(that.data.id==that.data.idi[j].id){
            that.setData({
              f1:that.data.idi[j].device_fee,
              f2:that.data.idi[j].service_fee,
              f4:that.data.idi[j].Single_end_average_fee
             })
        ...
        if(that.data.f1=="0"&that.data.f2=="0"&that.data.f4=="0"){
          wx.showToast({
            icon:"none",
            title:"该组合未存在"
          })
